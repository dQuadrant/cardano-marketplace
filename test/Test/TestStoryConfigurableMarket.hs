...
simpleMarketSpecs testIndex scriptName marketHelper context@(TestContext chainInfo networkId sKey walletAddr _ _ _ )  ioAction =
  let
      ...
      before ioAction $ do
        it "Should mint  4 Native Assets" $ \(configTx,saleTx, refUtxo) -> do
          runTest_ 1 Nothing "Mint Native Asset"  (do
              ...
              pure $     mintBuilder
                      <> txConsumeUtxos_ (UTxO $ Map.fromList $ take 100  $ Map.toList utxos)
